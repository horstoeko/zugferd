...
- Added `getDocumentBusinessProcess()` to `ZugferdDocumentReader` to read the business process context parameter (BT-23), providing a Reader counterpart to the Builder's `setDocumentBusinessProcess()`.
- Added `getDocumentForeignCurrency()` to `ZugferdDocumentReader` to read the foreign currency code (BT-6), foreign tax total amount (BT-X-260), and exchange rate, providing a Reader counterpart to the Builder's `setForeignCurrency()`.
...
- Renamed misspelled methods and variables for code clarity (old names remain as deprecated aliases where they are part of the public API):
  - `initSerialzer()` → `initSerializer()` in `ZugferdDocument` and `ZugferdDocumentProfileConverter`
  - `resolveScenatioZipFilename()` → `resolveScenarioZipFilename()` in `ZugferdKositValidator`
  - `$newVlidatorRuleset` → `$newValidatorRuleset` in `ZugferdPdfValidator`
  - `extractInvoiceInformations()` → `extractInvoiceInformation()` in `ZugferdDocumentPdfBuilderAbstract`
  - `validationPased()` → deprecated alias now delegates to correctly named `validationPassed()` in `ZugferdXsdValidator`
- Removed dead code: unused `$fileSpecDictionnaryIndex` property in `ZugferdPdfWriter` (was written but never read)
- Removed two `PHP_VERSION_ID >= 80000` conditional blocks in `ZugferdKositValidator`, since the minimum PHP version is now 8.3
- Converted `switch` statement to `match` expression in `ZugferdDocumentPdfBuilderAbstract::extractInvoiceInformation()` for PHP 8.3+ idiomatic code
- Added proper PHPDoc descriptions for 3 undocumented functions in `ZugferdObjectHelper` and 1 undocumented variable in `ZugferdDocumentReader`

### Fixed

- Fixed PDF/A output intent dictionary key `/OuputCondition` → `/OutputCondition` in `ZugferdPdfWriter`. The misspelling caused a non-standard PDF/A metadata key which could cause PDF/A validation failures.
- Fixed `getDocumentRoutingId()` in `ZugferdDocumentReader` using pass-by-value instead of pass-by-reference for the `$routingId` parameter. The method always returned an empty string to the caller because the result was never written back.
- Fixed `DocumentTest::testDocumentInternals` trying to access `$serializerBuilder` as a class property via Reflection, but it is a local variable inside `initSerializer()`. Removed the invalid assertion.
...
- Added `ReaderExtendedBuyerTaxRepresentativeTest` (10 tests) with round-trip tests for BuyerTaxRepresentativeParty (BG-X-54). The Builder creates an Extended-profile document with buyer tax representative data including name, global ID, tax registration, postal address, legal organisation, and two contacts, then the Reader verifies all fields are correctly read back. These tests were necessary because the feature was ported from an external fork (NicedriverMX) which provided no tests, and both the Builder and the newly added Reader methods needed coverage.
- Added `XsdValidatorTest` (4 tests) with dedicated tests for `ZugferdXsdValidator`, covering `validate()`, `hasNoValidationErrors()`, `hasValidationErrors()`, `validationErrors()`, the deprecated methods `validationPassed()`, `validationPased()`, `validationFailed()`, and fluent return type verification. Previously this class was only tested implicitly.
- Added `SpecificationVersionsTest` (4 tests) verifying that all version constants in `ZugferdSpecificationVersions` are defined, non-empty strings matching a semantic version format.
- Added `QuickDescriptorTest` (10 tests) covering the `ZugferdQuickDescriptor` base class and all profile-specific subclasses (`En16931`, `Extended`, `XRechnung`, `XRechnung2`, `XRechnung3`). Tests verify instance creation across all profiles, invoice and credit memo creation, automatic total calculation, payment terms, prepaid amounts, and fluent interface chaining. Previously these classes had no dedicated test coverage.
